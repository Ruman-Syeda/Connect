...
const createCommunity = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "communityManager" && req.user.role !== "admin") {
    console.log("User not authorized to create communities")
    res.status(403)
  ...
  if (communityExists) {
    console.log("Community with this name already exists")
    res.status(400)
    throw new Error("Community with this name already exists")
  }

  // Create the community without checking for limits
