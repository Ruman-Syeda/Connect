...
const CommunityManagerPage = () => {
  ...
  useEffect(() => {
    // Debug information
    console.log("CommunityManagerPage - User Info:", userInfo)
    setDebugInfo(`Role: ${userInfo?.role || "none"}`)
  ...
  useEffect(() => {
    if (success) {
      ...
      setImage("")
      setIsPrivate(false)
      // Refresh communities list after successful creation
      dispatch(getCommunities())
    }
  }, [success, dispatch])
  ...
  const handleCreateCommunity = (e) => {
    e.preventDefault()
    console.log("Creating community:", { name, description, image, isPrivate })
    dispatch(
      createCommunity({
        name,
        description,
  ...
  const filteredCommunities = communities?.filter((community) => {
    if (activeTab === "myCommunities") {
      return community.managers && community.managers.some((manager) => manager._id === userInfo._id)
    }
    return true
  })
